...
const employeesBonus = (arr) => {
    ...
    arr.forEach((element) => {
      if (element.workHours > 8) {
        bonus = parseInt(element.salary) + 100 + "$";
        element['salary'] = bonus;
      //   console.log("This is 100:", bonus);
      } else if (element.workHours <= 8) {
        bonus = parseInt(element.salary) + 50 + "$";
        element['salary'] = bonus;
      //   console.log("This is 50: ", bonus);
      }
    });
    return arr;
  };
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
  ...
  let maxPrice = 0;
  let maxMouse = null;
  let maxKeyBoard = null;
  
    for(let mouse of mouseArray){
