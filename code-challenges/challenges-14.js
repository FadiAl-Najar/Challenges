...
const LastWord = (str) => {
  ...
  const lastWord = str.lastIndexOf(" ");
  const sliceWord = str.slice(lastWord + 1);
  return sliceWord;
};
// -------------------------------------------------------------------------------------------------------
...
const replaceWords = (str) => {
  ...
  const am = word.indexOf("am");
  if (am !== -1) {
    word.splice(am, 1, "are");
  }

  const was = word.indexOf("was");
  if (was !== -1) {
    word.splice(was, 1, "were");
  }
...
const arrToStr = (arr) => {
  // write your code here
  let result = arr.join(" ");
  let stor = result.split(" ");

  for (let i = 4; i < stor.length; i += 5) {
    stor[i] += ",";
  }
